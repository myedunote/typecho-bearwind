			...
			<div class="col-mb-12 typecho-list">
				<div class="typecho-option-tabs">
					<ul class="typecho-option-tabs clearfix">
						<li class="current">
						   <form action="<?php $options->index('/action/shortlinkss?add'); ?>" method="post" >
				...
				<table class="typecho-list-table">
					...
					<tbody>
						...
						<?php $links = $db->fetchAll($db->select()->from('table.shortlinkss')->page($page, 15)->order('table.shortlinkss.id', Typecho_Db::SORT_DESC)); ?>
						...
						<tr class="even" id="<?php _e($link['id']); ?>" >						   
							...
							<td>
								...
								<a lang="<?php _e('你确认要删除该链接吗?'); ?>" href="<?php $options->index('/action/shortlinkss?del=' . $link['id']); ?>" class="operate-delete"><?php _e('删除'); ?></a>
							</td>
						</tr>
						<?php endforeach;?>
					</tbody>
				...
				<div class="typecho-pager">
					<div class="typecho-pager-content">
						<ul>							
							<?php $total = $db->fetchObject($db->select(array('COUNT(id)' => 'num'))->from('table.shortlinkss'))->num; ?>
							<?php for($i=1;$i<=ceil($total/15);$i++): ?>
							<li class='current'><a href="<?php $options->adminUrl('extending.php?panel=BearWindShortLinks%2Flinkchange.php&page='.$i); ?>" style= 'cursor:pointer;' title='第 <?php _e($i); ?> 页'> <?php _e($i); ?> </a></li>
							<?php endfor; ?>
...
$(document).ready(function () {
	$('.operate-edit').click(function () {
	   ...
	   $('#u-'+id).click(function(){
			$.ajax({
			   url: '<?php $options->index('/action/shortlinkss?edit'); ?>',
	...
	$('#qlinks').click(function(){	   
		$.ajax({
			url:'<?php $options->index('/action/shortlinkss?resetLink'); ?>',
			data:'link='+$('#links').val(),
			dataType:'json',
			success:function(data){
				if('success' === data){				   
